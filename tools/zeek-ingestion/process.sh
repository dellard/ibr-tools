...
# The utilities used by this script are in ../pcap-ingestion,
# not here.
#
INGEST_SCRIPTDIR="$SCRIPTDIR"/../pcap-ingestion

echo "$PNAME: Filling missing hours"
"$INGEST_SCRIPTDIR"/fill-missing.sh "$1" || exit 1

echo "$PNAME: Finding all subnets"
"$INGEST_SCRIPTDIR"/find-subnets.sh "$1" || exit 1

echo "$PNAME: Computing trends"
"$INGEST_SCRIPTDIR"/trend-by-subnet.sh "$1" || exit 1
